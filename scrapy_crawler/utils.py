...
def down_video(video_list, title, start_url, page):
    ...
    currentVideoPath = os.path.join('bilibili_video', title)  # 当前目录作为下载目录
...
# https://www.bilibili.com/video/BV1wk4y1r7UR?from=search&seid=839845467144348115
...
def save_video(uri):
    ...
    # https://www.bilibili.com/video/BV1wk4y1r7UR?from=search&seid=839845467144348115
    bv = re.findall('www.bilibili.com/video/(.*?)\?from=search', uri)

    start = dec(''.join(bv))  # input('请输入您要下载的B站av号或者视频链接地址:')
    start_url = 'https://api.bilibili.com/x/web-interface/view?aid=' + str(start)

    quality = 64  # 要下载视频的清晰度(1080p:80;720p:64;480p:32;360p:16)(填写80或64或32或16)
    # 获取视频的cid,title
    headers = {
        'User-Agent': 'Mozilla/5.0 (Windows NT 6.1; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/55.0.2883.87 Safari/537.36'
    }
